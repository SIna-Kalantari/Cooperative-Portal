...
function gregorian_to_jalali($gy, $gm, $gd, $mod = '')
{
	...
	$gy = tr_num($gy);
	$gm = tr_num($gm);
	$gd = tr_num($gd);/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
...
function jalali_to_gregorian($jy, $jm, $jd, $mod = ''){
	$jy = tr_num($jy);
	$jm = tr_num($jm);
	$jd = tr_num($jd);/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
	$gy = ($jy <= 979) ? 621 : 1600;
	$jy -= ($jy <= 979) ? 0 : 979;
...
function todayStartTimestamps(){
	$todayDate = jgetdate();
	return jmktime(0, 0, 0, $todayDate['mon'], $todayDate['mday'], $todayDate['year']);
}
